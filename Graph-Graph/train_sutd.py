...
parser.add_argument("--dataset_path", type=str, default="data/sutd/obj_feat", help="Path to extracted objects data")
parser.add_argument("--img_dataset_path", type=str, default="data/sutd/i3d_feat", help="Path to I3D feature data")
parser.add_argument("--obj_mapping_file", type=str, default="data/sutd/obj_idx_to_labels.json", help="path to object label mapping file")
parser.add_argument("--split_path", type=str, default="splits_sutd/", help="Path to train/test split")
parser.add_argument("--num_epochs", type=int, default=10, help="Number of training epochs")
parser.add_argument("--batch_size", type=int, default=32, help="Size of each training batch for frames")
...
parser.add_argument("--test_only", type=int, default=1, help="Test the loaded model only the video 0(False)/1(True)")
...
def test_model(epoch, model, test_dataloader):
	...
	for batch_i, (X, edge_index, y_true, img_feat, video_adj_list, edge_embeddings, temporal_adj_list, obj_vis_feat, batch_vec, toa) in enumerate(test_dataloader):
		...
		temporal_edge_w = temporal_edge_w.type(torch.float16)
	...
	if avg_prec > best_ap:
		best_ap = avg_prec
		os.makedirs("model_checkpoints/sutd", exist_ok=True)
		torch.save(model.state_dict(), f"model_checkpoints/sutd/{model.__class__.__name__}_{epoch}.pth")
		print(f"Saved the model checkpoint - model_checkpoints/sutd/{model.__class__.__name__}_{epoch}.pth")
...
def main():
	...
	if opt.checkpoint_model:
		state_dict = torch.load(opt.checkpoint_model, map_location=torch.device('cpu'))
		if 'pool.weight' in state_dict:
			state_dict['pool.select.weight'] = state_dict.pop('pool.weight')
