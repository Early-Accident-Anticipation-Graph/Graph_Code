...
class Dataset(Dataset):
	def __init__(self, dataset_path, img_dataset_path, split_path, ref_interval, objmap_file, training):
		...
		self.topk = 10
		self.frame_stats_path = "data/sutd/frames_stats"  #(height, width)
		self.n_frames = 100
	...
	def _extract_feature_paths(self, dataset_path, split_path="splits_sutd/", training=True):
	...
	def __getitem__(self, index):
		...
		if curr_vid_label > 0:        
			img_file = os.path.join(self.img_dataset_path, feature_path.split('/')[-2], "positive", feature_path.split('/')[-1].split(".")[0] + '.npy')
		else: 
			img_file = os.path.join(self.img_dataset_path, feature_path.split('/')[-2], "negative", feature_path.split('/')[-1].split(".")[0] + '.npy')            
		all_img_feat = self.transform(np.load(img_file)).squeeze(0)
        		
		#Reading frame stats file
		if curr_vid_label > 0:        
			frame_stats_file = os.path.join(self.frame_stats_path, feature_path.split('/')[-2], "positive", feature_path.split('/')[-1].split(".")[0] + '.npy')
		else:            
			frame_stats_file = os.path.join(self.frame_stats_path, feature_path.split('/')[-2], "negative", feature_path.split('/')[-1].split(".")[0] + '.npy')            
